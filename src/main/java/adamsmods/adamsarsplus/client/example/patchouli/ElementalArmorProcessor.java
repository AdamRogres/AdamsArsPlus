package adamsmods.adamsarsplus.client.example.patchouli;
    ...
    public @NotNull IVariable process(Level level, String key) {
        ...
        return switch (key) {
            case "reagent" -> IVariable.wrapList(Arrays.stream(recipe.reagent().getItems())
                    .map(ItemStack::copy)
                    .peek(i -> i.set(DataComponentRegistry.ARMOR_PERKS, i.getOrDefault(DataComponentRegistry.ARMOR_PERKS, new ArmorPerkHolder()).setTier(2)))
