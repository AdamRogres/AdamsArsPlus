...
import static com.adamsmods.adamsarsplus.entities.AdamsModEntities.HOLY_MAGE_ENTITY;

public class HolyMageEntity extends MysteriousMageEntity implements RangedAttackMob {

    public static final EntityDataAccessor<Boolean> CASTING =
            SynchedEntityData.defineId(HolyMageEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public static final EntityDataAccessor<Integer> INDEX =
            SynchedEntityData.defineId(HolyMageEntity.class, EntityDataSerializers.INT);

    public HolyMageEntity(EntityType<? extends Monster> pEntityType, Level pLevel){
        super(pEntityType, pLevel);
    }

    public HolyMageEntity(Level pLevel){
        this(HOLY_MAGE_ENTITY.get(), pLevel);
    }
            ...
            try {
                ...
                for(int i = 0; i < size; i++){
                    if(i + this.getIndex() < size){
                        if(CommunityMages.mages.get(i + this.getIndex()).tier.contains("holy")){
                        ...
                        if(CommunityMages.mages.get(i + this.getIndex() - size).tier.contains("holy")){
                            if(MAGES_GRIEF.get() || !CommunityMages.mages.get(i + this.getIndex()).tier.contains("griefing")){
                                Offset = i;
