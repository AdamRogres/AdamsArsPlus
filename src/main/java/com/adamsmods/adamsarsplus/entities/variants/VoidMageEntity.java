...
import static com.adamsmods.adamsarsplus.entities.AdamsModEntities.VOID_MAGE_ENTITY;

public class VoidMageEntity extends MysteriousMageEntity implements RangedAttackMob {

    public static final EntityDataAccessor<Boolean> CASTING =
            SynchedEntityData.defineId(VoidMageEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public static final EntityDataAccessor<Integer> INDEX =
            SynchedEntityData.defineId(VoidMageEntity.class, EntityDataSerializers.INT);

    public VoidMageEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public VoidMageEntity(Level pLevel) {
        this(VOID_MAGE_ENTITY.get(), pLevel);
    }
    ...
    public void setSelfCasting(boolean scasting) {
        this.entityData.set(SELF_CASTING, scasting);
    }

    public boolean isSelfCasting() {
        return this.entityData.get(SELF_CASTING);
    }
    ...
    public void setIndex(Integer index) {
        this.entityData.set(INDEX, index, true);
    }
            ...
            try {
                ...
                for(int i = 0; i < size; i++){
                    if(i + this.getIndex() < size){
                        if(CommunityMages.mages.get(i + this.getIndex()).tier.contains("void")){
                        ...
                        if(CommunityMages.mages.get(i + this.getIndex() - size).tier.contains("void")){
                            if(MAGES_GRIEF.get() || !CommunityMages.mages.get(i + this.getIndex()).tier.contains("griefing")){
                                Offset = i;
                ...
                try {
                    this.setIndex(12);
                    CommunityMages.ComMages communityMage = (CommunityMages.ComMages)CommunityMages.mages.get(12);
