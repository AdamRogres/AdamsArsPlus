...
import static com.adamsmods.adamsarsplus.entities.AdamsModEntities.FROST_MAGE_ENTITY;

public class FrostMageEntity extends MysteriousMageEntity implements RangedAttackMob {

    public static final EntityDataAccessor<Boolean> CASTING =
            SynchedEntityData.defineId(FrostMageEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public static final EntityDataAccessor<Integer> INDEX =
            SynchedEntityData.defineId(FrostMageEntity.class, EntityDataSerializers.INT);

    public FrostMageEntity(EntityType<? extends Monster> pEntityType, Level pLevel){
        super(pEntityType, pLevel);
    }

    public FrostMageEntity(Level pLevel){
        this(FROST_MAGE_ENTITY.get(), pLevel);
    }
            ...
            try {
                ...
                for(int i = 0; i < size; i++){
                    if(i + this.getIndex() < size){
                        if(CommunityMages.mages.get(i + this.getIndex()).tier.contains("frost")){
                        ...
                        if(CommunityMages.mages.get(i + this.getIndex() - size).tier.contains("frost")){
                            if(MAGES_GRIEF.get() || !CommunityMages.mages.get(i + this.getIndex()).tier.contains("griefing")){
                                Offset = i;
                ...
                try {
                    this.setIndex(8);
                    CommunityMages.ComMages communityMage = (CommunityMages.ComMages)CommunityMages.mages.get(8);
